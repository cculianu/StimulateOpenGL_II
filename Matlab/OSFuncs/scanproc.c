...
pid_t *pids_of_exe(const char *exe)
{
  ...
  while(n--) {
    ...
    if (!full_path) {
        /* parse out just the basename without the path*/
        char *slash = strrchr(link, '/');
        if (slash && *(slash+1) ) {
            int newlen;
            *slash = 0;
            newlen = strlen(slash+1);
            memmove(link, slash+1, newlen);
            link[newlen] = 0;
        }
    }
    /* 
...
const struct ModList * get_module_list(void)
{
  ...
  if (f) {
    ...
    while (fgets(line, 4096, f)) {
      ...
      if (!tok) continue;      
      len = strlen(tok);
      ml = (struct ModList *)malloc(sizeof(struct ModList));      
      memset(ml, 0, sizeof(struct ModList));
      if (prev_ml) prev_ml->next = ml;
      ml->mod = (char *)malloc(len+1);
      ...
      do {
        ...
        do {
          ...
          ml->refs = (char **)realloc(ml->refs, sizeof(char *) * ++ml->n_refs);
          ml->refs[ml->n_refs-1] = (char *)malloc(sizeof(char)*(len+1));
