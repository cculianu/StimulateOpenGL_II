...
public:
    ...
    int decode(QImage *image, const uchar* buffer, int length,
               int *nextFrameDelay, int *loopCount, bool rahack = false);
	
	bool headerWasRead() const { return headerwasread; }
	
    static void scan(QIODevice *device, QVector<QSize> *imageSizes, QVector<QPoint> *topCorners, int *loopCount, QVector<int> *imageOffsets, QVector<int> *imageLengths);
...
private:
    ...
    bool digress;
	bool headerwasread;
    void nextY(unsigned char *bits, int bpl, bool rahack = false);
    void disposePrevious(QImage *image);
};

/*!
    Constructs a GIFFormat.
*/
GIFFormat::GIFFormat()
{
    globalcmap = 0;
...
void GIFFormat::disposePrevious(QImage *image)
{
    if (out_of_bounds) {
...
*/
int GIFFormat::decode(QImage *image, const uchar *buffer, int length,
                       int *nextFrameDelay, int *loopCount, bool rahack)
{
        ...
        switch (state) {
          ...
          case ImageDescriptor:
            ...
            if (count==10) {
                ...
                if (image->isNull()) {
					if (rahack) format = QImage::Format_Indexed8;
                ...
                if (!rahack) disposePrevious(image);
                disposed = false;
                ...
                if (frame == 0 && !rahack) {
					...
					if (trans_index >= 0)
						memset(bits, (unsigned char)trans_index, image->byteCount());
					else if (bgcol >= 0)
						memset(bits, (unsigned char)bgcol, image->byteCount());
					else
						memset(bits, 0, image->byteCount());
				}

                if (!rahack && disposal == RestoreImage) {
                    int l = qMin(swidth-1,left);
                    int r = qMin(swidth-1,right);
                    int t = qMin(sheight-1,top);
                    int b = qMin(sheight-1,bottom);
                    ...
                    if (needfirst) {
                        ...
                        if (!out_of_bounds && image->height() > y && firstcode!=trans_index) {
							if (!rahack)
								...
								((unsigned char*)FAST_SCAN_LINE(bits, bpl, y))[x] = firstcode;								
						}
                        x++;
                        ...
                        *sp++=firstcode=table[1][code];
                        ...
                        while (sp>stack) {
                            ...
                            if (!out_of_bounds && h > y && index!=trans_index) {
								if (rahack) {
									unsigned char *ln = (unsigned char *)line;
									if (index > ncols)
										ln[x] = 255;
									else
										ln[x] = index; //(unsigned char)qGray(map ? map[index] : 0);
								} else {
									if (index > ncols)
...
*/
void GIFFormat::scan(QIODevice *device, QVector<QSize> *imageSizes, QVector<QPoint> *topCorners, int *loopCount, QVector<int> *imageOffsets, QVector<int> *imageLengths)
{
    if (!device)
        return;
        ...
        while (!done && length) {
            ...
            case ImageDescriptor:
                ...
                if (count == 10) {
					...
					*topCorners << QPoint(newLeft, newTop);
            ...
            case ImageDataBlockSize:
                ...
                if (blockSize) {
					...
					// end image!
					// save image length
					imageLengths->push_back( 
											(device->pos() - length) 
											- (imageOffsets->size() ? imageOffsets->back() : 0)
											);
            ...
            case ExtensionLabel:
                switch (ch) {
                case 0xf9:
						imageOffsets->push_back(device->pos()-length-2);
						state = GraphicControlExtension;
                    break;
                case 0xff:
                    state = ApplicationExtension;
...
void GIFFormat::fillRect(QImage *image, int col, int row, int w, int h, QRgb color)
{
    if (w>0) {
        for (int j=0; j<h; j++) {
...
void GIFFormat::nextY(unsigned char *bits, int bpl, bool rahack)
{
    int my;
    switch (interlace) {
    ...
    case 1: {
        ...
        if (trans_index < 0) {
            for (i=1; i<=my; i++) {
                memcpy(FAST_SCAN_LINE(bits, bpl, y+i)+left*(rahack ? 1 : sizeof(QRgb)), 
					   FAST_SCAN_LINE(bits, bpl, y)+left*(rahack ? 1 : sizeof(QRgb)),
                       (right-left+1)*(rahack ? 1 : sizeof(QRgb)));
            }
        }
...
inline QRgb GIFFormat::color(uchar index) const
{
...
bool GifReader::imageIsComing() const
{
...
bool GifReader::canRead(QIODevice *device)
{
    if (!device) {
        qWarning("GifReader::canRead() called with no device");
        return false;
    }
...
bool GifReader::read(QImage *image)
{
    const int GifChunkSize = 4096;
...
bool GifReader::randomAccessRead(QImage *image, int imgnum)
{
	...
	QByteArray buffer;
	if (!gifFormat->headerWasRead()) {
		buffer.resize(imageOffsets.first());
		if ( device()->read((char *)buffer.data(), buffer.size()) < buffer.size() ) {
			device()->seek(oldPos);
			return false;
		}
		int decoded = gifFormat->decode(image, (const uchar *)buffer.constData(), buffer.size(), &nextDelay, &loopCnt, true);
		if (decoded != buffer.size()) {
				// ??
			qDebug("decode of header returned %d, epxected %d", decoded, buffer.size());
		}
	}
	...
	if (decoded != buffer.size()) {
		// ?? wtf
		qDebug("decode of image returned %d, epxected %d", decoded, buffer.size());
	}	
...
bool GifReader::write(const QImage &image)
{
    Q_UNUSED(image);
    return false;
}

bool GifReader::supportsOption(ImageOption option) const
{
...
bool GifReader::isAnimatedGifNonOptimized() const
{
	if (!scanIsCached) {
		GIFFormat::scan(device(), &imageSizes, &topCorners, &loopCnt, &imageOffsets, &imageLengths);
		scanIsCached = true;
	}
	//debug print image lengths..
	//for (int i = 0; i < (int)imageLengths.size(); ++i) {
	//	qDebug("img %d offset: %d (0x%x) length: %d (0x%x)", i+1, imageOffsets[i], imageOffsets[i], imageLengths[i], imageLengths[i]);
	//}
	if (!isOptimizedIsCached) {
		isOptimized = false;
		for (int i = 0; !isOptimized && i < (int)topCorners.size(); ++i) {
				if (topCorners[i].x() != 0 && topCorners[i].y() != 0)
					isOptimized = true;
		}
		isOptimizedIsCached = true;
	}
	return !isOptimized;
}

QVariant GifReader::option(ImageOption option) const
{
    if (option == Size) {
        if (!scanIsCached) {
            GIFFormat::scan(device(), &imageSizes, &topCorners, &loopCnt, &imageOffsets, &imageLengths);
...
void GifReader::setOption(ImageOption option, const QVariant &value)
{
    Q_UNUSED(option);
    Q_UNUSED(value);
}

int GifReader::nextImageDelay() const
{
    return nextDelay;
}

int GifReader::imageCount() const
{
    if (!scanIsCached) {
        GIFFormat::scan(device(), &imageSizes, &topCorners, &loopCnt, &imageOffsets, &imageLengths);
        scanIsCached = true;
...
int GifReader::currentImageNumber() const
{
    return frameNumber;
}

QByteArray GifReader::name() const
{
    return "gif";
}


void GifReader::copyImageLengthsAndOffsets(const GifReader & o)
{
	imageSizes = o.imageSizes;
	loopCnt = o.loopCnt;
	topCorners = o.topCorners;
	imageOffsets = o.imageOffsets;
	imageLengths = o.imageLengths;
	scanIsCached = o.scanIsCached;
	isOptimizedIsCached = o.isOptimizedIsCached;
